...
| 持续吞吐、峰值吞吐及持续时间 | 冻结持续目标 >= 10 events/s，steady 10s；本次端到端 steady 结果 17.2 events/s；latency input 结果 547.0k events/s 仅作峰值入口观测，不作为端到端容量承诺 |
...
- 当前工作：本轮第 1、6 项已完成；后续容量承诺必须在目标部署硬件上重新运行同一验收脚本，不得把本机结果外推为任意规模保证。
...
- 本轮入口修复：`DriverCoordinator::shutdown_gracefully` 先写入 Driver `Drain` intent，执行 live source drain 并等待 `Succeeded/Failed`，`cmd/coordinator` 通过 `moonbitlang/async/signal` 注册 SIGTERM/SIGINT；`moon check --target native`、`moon build --target native cmd/coordinator` 和 `driver_coordinator_e2e_test` 通过。独立进程证据：`moon run scripts/signal_acceptance.mbtx` 使用现有 `cmd/driver_fixture/process-submission.json` 与 `process-events.json` 启动三个 native 进程，Driver 返回 `accepted=1`，仅向 Coordinator 发送 SIGTERM 后 Coordinator 记录 `phase=Succeeded`，等待状态 `-15`（shell 语义 143），Worker 退出 0；脚本同时检查无清理兜底及 Worker 的一批持久化执行日志。
...
- 本轮性能与故障证据：`scripts/production_acceptance.mbtx` 冻结硬件/拓扑/负载/阈值，启动真实 Coordinator、Worker、Driver，运行 64 次 latency、10 秒 steady、终态 drain，随后验证不可达 Driver 端点、4096-byte WAL capacity refusal，并调用真实 Kafka 4.0.0 多分区/consumer-group/broker restart 联调。最近一次证据：`/tmp/moonweave-production-acceptance-.38716.289e5ecf/production-evidence.txt`；全流程命令：`moon run scripts/production_acceptance.mbtx /tmp/kafka_2.13-4.0.0`。
